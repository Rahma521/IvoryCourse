...
class SettingsController extends Controller
{
    ...
    public function updateSettings(Request $request)
    {
        ...
        if (!isset($request['WeekEndThu'])) {
            $setting = Settings::where('key', 'WeekEndThu')->first();
            if ($setting == '') {
                $setting = New Settings;
                $setting->key = 'WeekEndThu';
                $setting->save();
            }
            $setting->value = '0';
            $setting->update();
        }
        if (!isset($request['WeekEndFri'])) {
            $setting = Settings::where('key', 'WeekEndFri')->first();
            if ($setting == '') {
                $setting = New Settings;
                $setting->key = 'WeekEndFri';
                $setting->save();
            }
            $setting->value = '0';
            $setting->update();
        }
        if (!isset($request['WeekEndSat'])) {
            $setting = Settings::where('key', 'WeekEndSat')->first();
            if ($setting == '') {
                $setting = New Settings;
                $setting->key = 'WeekEndSat';
                $setting->save();
            }
            $setting->value = '0';
            $setting->update();
